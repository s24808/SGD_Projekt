...
void addBird(SDL_Renderer* renderer){
    Obstacle bird;
    bird.x = SCREEN_WIDTH;
    bird.y = dinoY; //Dodanie na wysokości głowy dinozaura
...
std::vector<SDL_Texture*> loadNumberTextures(SDL_Renderer* renderer) {
    ...
    for (int i = 0; i < 10; i++) {
        std::string filePath = "C:\\Users\\Filip\\Desktop\\SGD_Projekt\\assets\\" + std::to_string(i) + ".bmp"; //Załadowanie obrazków bmp cyfr (ścieżkę prawdopodbnie trzeba będzie zmienić na zajęciach)
...
int main(int argc, char* args[]) {
    ...
    birdTexture = loadTexture("C:\\Users\\Filip\\Desktop\\SGD_Projekt\\assets\\bird.bmp", renderer); //Załadowanie obrazka bmp ptaka (ścieżkę prawdopodbnie trzeba będzie zmienić na zajęciach)
